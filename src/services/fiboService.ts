import { RenderSettings, CameraState } from '../types.ts';
...
export const generateSceneImageFIBO = async (
  ...
  const apiKey = (process.env.VITE_FIBO_API_KEY as string) || "";
  if (!apiKey) {
  ...
  const generatePathRaw =
    (process.env.VITE_FIBO_GENERATE_PATH as string) || "/fibo/image/generate";
  ...
  for (const path of candidatePaths) {
    try {
      onProgress?.(`Enviando petición a ${path}`);
      ...
      const preferred = (process.env.VITE_FIBO_AUTH_HEADER as string) || "api_token";
      ...
      if (!resp.ok) {
        ...
        if (resp.status === 401) {
          lastError = new Error(
            `Unauthorized (401): Verify VITE_FIBO_API_KEY and required auth header. Try setting VITE_FIBO_AUTH_HEADER to one of authorization|api_token|x-api-key|x-api-token.`
          );
        }
        throw lastError;
      }
  ...
  if (statusUrl) {
    onProgress?.('Petición asíncrona aceptada, monitorizando estado...');
    ...
    for (let i = 0; i < maxAttempts; i++) {
      onProgress?.(`Poll intento ${i + 1}/${maxAttempts}...`);
      ...
      if (status) {
        onProgress?.(`Estado: ${status}`);
        if (["failed", "error"].includes(status.toLowerCase())) {
          throw new Error(`FIBO generation failed at status endpoint. Body: ${sText}`);
        }
      }

      // if server provides progress percentage, surface it
      const percent = sData?.progress || sData?.result?.progress || sData?.data?.progress;
      if (typeof percent === 'number') {
        onProgress?.(`Progreso: ${Math.round(percent * 100)}%`);
      }
